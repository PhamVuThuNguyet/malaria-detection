...
# importing functions to load image and covert to array
...
df_test.img_name = df_test.img_name.apply(
    lambda x: "annotated_data/testing_images/"+str(x))
            ...
            for img in tqdm(testing_images):
                ...
                for (box, score, label) in zip(boxes, scores, labels):
                    ...
                    if img in predicted_boxes_stacked_test:
                        predicted_boxes_stacked_test[img]['boxes'].append(
                            [startX, startY, endX, endY])
                        predicted_boxes_stacked_test[img]['scores'].append(
                            score)
                    else:
                        predicted_boxes_stacked_test[img]['boxes'] = [
                            [startX, startY, endX, endY]]
                    ...
                    if categoryIdx[label]['name'] == "non_rbc":
                        if categoryIdx[label]['name'] in testing_predicition_rnn:
                            testing_predicition_rnn[categoryIdx[label]
                                                    ['name']] += 1
                        else:
                            testing_predicition_rnn[categoryIdx[label]
                                                    ['name']] = 1
...
print("Ground Truth for Finetuned VGG-16::",
      dict(df_test_all.label.value_counts()))
